...
/*   Updated: 2024/07/07 11:16:31 by Guillem Barulls  ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*new_line(char *str_gnl, char *str_tmp, int fd)
{
	static char	*temp;
	char		*str_gnl_tmp;
	int			total_len;
	...
	while (control == 0)
	{
		...
		if (!str_gnl)
			return (NULL);
		str_gnl = ft_strjoin(str_gnl_tmp, str_tmp, temp, &control);
	}
	free(str_gnl_tmp);
	free(str_tmp);
	return (str_gnl);
...
char	*get_next_line(int fd)
{
	...
	*str_gnl = '\0';
	...
	if (!str_tmp)
		return (NULL);
	str_gnl = new_line(str_gnl, str_tmp, fd);
	return (str_gnl);
}
